...
# Email setup
EMAIL_BASE_URL = os.environ.get('EMAIL_BASE_URL', 'https://api.resend.com')
...
async def submit_recovery_appeal(
    ...
    if not EMAIL_API_KEY:
        raise HTTPException(
            status_code=500,
            detail="Email service not configured (EMAIL_API_KEY missing).",
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as http_client:
            resp = await http_client.post(
                ...
                headers={"X-Email-Key": EMAIL_API_KEY},
                json=email_payload,
            )
